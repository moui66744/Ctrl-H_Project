...
public class VariableInfo extends DeclarationBaseInfo {
    ...
    public static List<VariableInfo> variableInfoFilter(List<VariableInfo> variableInfos, JavaParser.TypeTypeContext type, boolean filterMode) {
        ...
        for (var variableInfo : variableInfos) { // 对列表中的每个变量声明信息类实例
            if (filterMode == Objects.equals(variableInfo.Type, type)) { // TODO: 2022/5/9 如果其与所需的类型符合
    ...
    public static List<VariableInfo> variableInfoTypeStringFilter(List<VariableInfo> variableInfos, String type, boolean filterMode) {
        List<VariableInfo> ret = new ArrayList<>();
        for (var variableInfo : variableInfos) { // 对列表中的每个变量声明信息类实例
            if (filterMode == Objects.equals(variableInfo.Type.getText(), type)) { // 其类型的字符串形式是否与过滤条件符合
                ret.add(variableInfo); // 通过过滤，将其作为待返回列表的一部分
            }
        }
        return ret;
